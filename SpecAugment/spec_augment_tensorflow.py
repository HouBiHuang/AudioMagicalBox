...
def time_masking(mel_spectrogram, time_masking_para=4, time_mask_num=1):
    ...
    for i in range(time_mask_num):
        #產生遮蔽範圍
        t = tf.random.uniform([], minval=0, maxval=time_masking_para, dtype=tf.int32) #遮罩範圍
        tau = tf.cast(tau, dtype=tf.int32) #總時間
        t0 = tf.random.uniform([], minval=0, maxval=tau-t, dtype=tf.int32) #遮罩後剩下的範圍

        # warped_mel_spectrogram[:, t0:t0 + t] = 0
        mask = tf.concat((tf.ones(shape=(1, n, tau - t0 - t, 1)),
                          tf.zeros(shape=(1, n, t, 1)), #用0遮罩
...
def spec_augment(mel_spectrogram):
    ...
    #時間遮罩
    warped_time_spectrogram = time_masking(mel_spectrogram)

    #頻率遮罩
    #warped_time_frequency_sepctrogram = frequency_masking(mel_spectrogram)

    #重整shape到原本模樣
    warped_time_frequency_sepctrogram = np.reshape(warped_time_spectrogram, (v, tau))
